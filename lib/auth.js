...
if (!SESSION_SECRET || SESSION_SECRET.length < 16) {
  if (process.env.NODE_ENV === 'production') {
    ...
    console.error(
      '\nMissing or too-short SESSION_SECRET.\n' +
      'Set a long random value in your host\'s environment variables, e.g.: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"\n'
    );
    process.exit(1);
  }
  ...
  // eslint-disable-next-line no-console
  console.warn('SESSION_SECRET not set — using a temporary one for this run (dev mode only).');
}
...
function createSessionToken({ salonId, adminId, email }) {
  const payload = { salonId, adminId, email, exp: Date.now() + SESSION_TTL_MS };
