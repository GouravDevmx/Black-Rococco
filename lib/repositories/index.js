...
class StaffRepository extends BaseRepository {
  constructor(db) {
    super(db, 'staff', { idPrefix: 'stf', counterKey: 'staff', label: 'Miembro del equipo' });
  }
  active() {
    return this.list(m => m.active).sort((a, b) => (a.sort || 0) - (b.sort || 0));
  }
  withPhoto() {
    return this.active().filter(m => m.photoUrl);
  }
}

// ADMIN ONLY. These are photos of a real client's hands/nails. Nothing in this
// repository may ever be reached from a public route — see public-config.js,
// which deliberately does not expose it.
class ClientPhotoRepository extends BaseRepository {
  constructor(db) {
    super(db, 'clientPhotos', { idPrefix: 'cph', counterKey: 'clientPhoto', timestamps: false, label: 'Foto de clienta' });
  }
  byClient(clientId) {
    return this.list(p => p.clientId === clientId)
      .sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
  }
  byAppointment(appointmentId) {
    return this.list(p => p.appointmentId === appointmentId);
  }
  countForClient(clientId) {
    return this.count(p => p.clientId === clientId);
  }
  // Called when a client is deleted, so photos are never orphaned. (Postgres
  // also cascades, but the in-memory array must be kept consistent too.)
  removeForClient(clientId) {
    return this.removeBy(p => p.clientId === clientId);
  }
}
...
function repositories(db) {
  return {
    ...
    clientPhotos: new ClientPhotoRepository(db)
  };
}
