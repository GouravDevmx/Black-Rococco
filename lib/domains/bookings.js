...
const { writeDb, insertAppointmentAtomic, upsertClientAndGetId, markPersisted } = require('../db');
...
// STORY 4 — the SINGLE booking path. Admin (walk-in / phone / WhatsApp) and
// customer bookings both land here on purpose, so they cannot drift apart.
//
// `isAdmin` deliberately does NOT relax any validation: the availability
// pre-check, the promotion rules and — crucially — the atomic slot insert are
// identical for both. It exists only to let non-validation concerns differ
// later (e.g. which notification copy is sent). If you ever find yourself
// writing `if (isAdmin) skip...` around a validation, stop: that is exactly
// the bug this consolidation was built to prevent, and it would let an admin
// double-book over a customer.
async function createBooking({ req, res, db, salonId, isAdmin = false }) {
  ...
  if (USE_SUPABASE && isNewClient) {
    ...
    client.id = realId;
    // Already written to Postgres by the upsert above — fold into the snapshot
    // so writeDb()'s diff treats it as existing (and only UPDATEs it if the
    // request mutates it further) rather than INSERTing a duplicate.
    markPersisted(db, 'clients', client);
  }
  ...
  // This row is ALREADY in Postgres (inserted atomically above), so fold it
  // into the read snapshot. Without this, the record-level diff in writeDb()
  // would see it as brand new and INSERT it a second time.
  if (USE_SUPABASE) markPersisted(db, 'appointments', appt);
