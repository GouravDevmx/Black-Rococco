...
const {
  USE_SUPABASE, supabase, SITE_URL, BUSINESS_TIME_ZONE,
...
function registerBookingNotifications(db, appt) {
  const dispatches = [];
  const appointment = publicAppointment(db, appt);
  ...
  const calendarNotification = addNotification(db, {
    ...
    message: GOOGLE_CALENDAR_WEBHOOK_URL ? 'Evento enviado a integración de Google Calendar.' : 'Configura GOOGLE_CALENDAR_WEBHOOK_URL para crear eventos automáticamente.',
    appointmentId: appt.id,
...
async function processClientReminders() {
  if (USE_SUPABASE) {
    const { data: salons, error } = await supabase.from('salons').select('id').eq('active', true);
    if (error) {
      console.error('processClientReminders: could not list salons:', error.message);
      return;
    }
    for (const salon of salons || []) {
      await processClientRemindersForSalon(salon.id);
    }
  } else {
    await processClientRemindersForSalon(null);
  }
}

async function processClientRemindersForSalon(salonId) {
  let db;
  const dispatches = [];
  let changed = false;
  try {
