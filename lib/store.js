...
function jsToRow(obj, fields, extra) {
  const out = { ...extra };
  for (const [jsKey, sqlKey] of fields) {
    // Omit the field entirely when it's undefined, so Postgres applies the
    // column's own default (e.g. error text default ''). Sending an
    // explicit null instead — as this used to do — is rejected by any
    // NOT NULL column regardless of its default; defaults only kick in
    // when a column is left out of the statement altogether. Explicit
    // null (as opposed to undefined) is left as-is on purpose, since some
    // columns (applied_promotion, start_date, ...) are genuinely nullable
    // and mean something by being null.
...
async function readDb(salonId, dbPath) {
  ...
  return {
    settings: {
      ...
      salonConfig: salonRow.salon_config || {},
      notifications: {
        adminPanel: true,
...
// Persists a client row RIGHT NOW instead of waiting for the generic
// writeDb() sync at the end of the request. Needed specifically because a
// brand-new client must exist in the database BEFORE the appointment that
// references it is inserted (otherwise Postgres correctly rejects the
// appointment — a foreign key can't point at a row that isn't there yet).
// Upserts on (salon_id, whatsapp) rather than id, so if two requests from
// the same new phone number land at the same instant, they merge into one
// client instead of erroring — and returns whichever id actually won, so
// the appointment is created against the real, persisted row.
async function upsertClientAndGetId(salonId, client) {
  ...
  const { data, error } = await supabase
    .from('clients')
    .upsert(row, { onConflict: 'salon_id,whatsapp' })
    .select()
    .single();
